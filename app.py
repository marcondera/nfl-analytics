...
def display_final_results_styled(df_finalized):
    ...
    Layout compacto e sem a tag "nfl-card" e o espaço vago.
    """
    
    rows = [row for index, row in df_finalized.iterrows()]
    
    # CSS com ajustes para layout mais compacto, SEM A CLASSE 'nfl-card'
    st.markdown("""
        <style>
            .stApp {
    ...
    for i in range(0, len(rows), 3):
        ...
        for j, row in enumerate(chunk):
            with cols[j]:
                ...
                # --- START GAME CONTAINER (SUBSTITUIU O nfl-card) ---
