...
st.set_page_config(
    ...
    layout="wide",
    initial_sidebar_state="collapsed" # Remove a barra lateral para economizar espaço
)
...
# --- 2. FUNÇÕES DE PROCESSAMENTO DE DADOS (Inalteradas, pois funcionam) ---
...
# --- 3. FUNÇÃO DE RENDERIZAÇÃO CUSTOMIZADA (GRID 3x1 - Design FINAL) ---
...
def main():
    ...
    # Injeta CSS Global para o fundo
    st.markdown("""
        <style>
            h1 {
    ...
    if not df_in_progress.empty:
        ...
        st.markdown('<p style="color:#888;">Nenhum jogo em andamento no momento.</p>', unsafe_allow_html=True)
    ...
    if not df_finalized.empty:
        ...
        st.markdown('<p style="color:#888;">Nenhum resultado finalizado encontrado.</p>', unsafe_allow_html=True)
    ...
    if not df_scheduled.empty:
        ...
        st.markdown('<p style="color:#888;">Nenhum jogo agendado nos dados fornecidos.</p>', unsafe_allow_html=True)
